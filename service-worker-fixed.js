...
self.addEventListener('activate', event => {
  console.log('[Service Worker] Activating KML-Fixed version...');
  event.waitUntil(
...
self.addEventListener('fetch', event => {
  ...
  // Special handling for KML files - ENHANCED FOR OFFLINE
  if (requestURL.includes('/kml_normalized/') || requestURL.includes('.kml')) {
    console.log('[Service Worker] KML request for:', requestURL);
    
    event.respondWith(
      caches.match(event.request)
        .then(cachedResponse => {
          if (cachedResponse) {
            console.log('[Service Worker] ✅ Returning cached KML file:', requestURL);
            return cachedResponse;
          }
          
          // If not in cache, try to fetch and cache
          console.log('[Service Worker] KML cache miss, fetching from network:', requestURL);
          return fetch(event.request.clone())
            .then(response => {
              if (response && response.status === 200) {
                // Cache the KML file for future offline use
                const responseToCache = response.clone();
                caches.open(CACHE_NAME)
                  .then(cache => {
                    console.log('[Service Worker] Caching new KML file:', requestURL);
            ...
            .catch(error => {
              console.error('[Service Worker] ❌ KML fetch failed (offline?):', requestURL, error);
              // Return a meaningful error for KML files
              return new Response(
                JSON.stringify({ error: 'KML file not available offline', url: requestURL }),
                { 
                  status: 404, 
    ...
    return; // Exit early after handling KML request
  }
...
console.log('[Service Worker] KML-Fixed Script loaded - Version 5');
